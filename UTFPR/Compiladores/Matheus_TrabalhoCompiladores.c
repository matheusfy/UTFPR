...
int main (int argc, char *argv[])
{
    ...
    while((ch = fgetc(arq_palavras)) != EOF) // verificaçao enquanto não é o final do arquivo
      {
        ...
        else if( ch == 10 || ch == 32  ) // verifica se espaço ou quebra de linha
        {
            ...
            if(flag == 0 && v[0] != '}' )
            {
              ...
              aux->linha = linha;
              aux->info[0] = '\0';
              *aux = compara(v,aux,&erro); // recebe nó tratado na funcao
  ...
  if(erro == 1) // se erro = 1, erro na etapa lexica
  {
    printf("fechando programa por erro na etapa lexica\n");
    return 0;
  }
  else
  {
    ...
    aux->linha = linha;
    strcpy(aux->info, "$");
    Inserir(&palavras,aux); // sifrao no final da fita
    imprimir(&palavras);

    /**********PARTE SINTATICA *******/
    Lista pilha;
    Definir (&pilha);
    if (tabela(&palavras, &pilha)==1) // entra no tratamento da etapa sintática
      printf("Erro na estapa Sintática\n"); 
    else
      printf("Etapa Sintática Aceita\n");
